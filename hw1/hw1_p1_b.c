...
bool hasHigherPriority(const char* op1, const char* op2);
char* readInputString(FILE* fp, size_t size);
bool isOperator(char c);

const char* operators = "+-*/";
const int priority[] = {'0', '0', '1', '1'};
// Testcase of infix to postfix
    //char inputBuffer[] = "A*B-C/D";      //AB*CD/-
    //char inputBuffer[] = "A+B-C*D/E";    //AB+CD*E/-
    //char inputBuffer[] = "A+B*C-D/E+F";  //ABC*+DE/-F+
...
int main(){
    ...
    // Buffer for answer
    char* output = (char*) malloc(inputLen + 1); // +1 for \0
    output[0] = '\0';

    size_t len1, len2;
    for (int i = 0; i < inputLen; i++) {
        if (isOperator(inputBuffer[i])) {
            while( !isStackEmpty(s) && hasHigherPriority(peekStack(s), &inputBuffer[i]) ){
                strcat(output, (char*)pop(s));
            }

            char operateStr[] = {inputBuffer[i], '\0'}; 
            push(s, Obj(operateStr, strlen(operateStr)));
        } 
        else {//operand
            char operandStr[] = {inputBuffer[i], '\0'}; 
            strcat(output, operandStr);
        }
    }
...
/**
 * @return priority of op1 > priority op2 
 */
bool hasHigherPriority(const char* op1, const char* op2){
    //operators = "+-*/"
    //'+', '-' : 0 ,   '*', '/' : 1 (higher)

    //strchr(operators, op1[0]) finds the first occurrence of op1[0] 
    //(the first character of the operator string) in the operators string and returns a pointer to that position.
    //By subtracting the base address of operators, you get the index of the operator.
    int index1 = strchr(operators, op1[0]) - operators;
    int index2 = strchr(operators, op2[0]) - operators;
